...
# Seconds given to the camera to settle after the first sequence is armed. The
# ring buffer is already filling during this pause, so it counts towards the
# pre-MOI window of sequence 0.
_ARM_SETTLE_S = 0.5
...
class Camera:
    ...
    def buffer_record(
        ...
        *,
        ...
        wait_for : float or callable or None, keyword-only, optional
            ...
            A number is a delay in seconds.  A callable is called once per
            sequence, with no arguments, and must return when the moment of
            interest has happened; its return value is ignored (for example
            ``input`` or ``lambda: my_event.wait()``).  ``None`` (default)
            fires the MOI as soon as the camera is ready; with
            ``pre_moi > 0`` it warns, because the MOI is then placed by a
            timer and not by an event, and waits until ``pre_moi /
            frame_rate`` seconds have passed since arming, so the
            pre-trigger window is full.  Pass ``wait_for=0`` to fire
            immediately without the warning.
        ...
        -----
        UserWarning
            If ``pre_moi > 0`` and no *wait_for* is given, or if *wait_for*
            is too short for the pre-trigger window to be full.  The first
            sequence also gets the 0.5 s settle pause after arming, during
            which the ring is already filling; later sequences do not.
        ...
        Same, but with a fixed 0.5 s delay after arming:
        ...
        if wait_for is None and pre_moi > 0:
            warnings.warn(
                ...
                f"by an event. buffer_record() waits {fill_time:.3f} s to fill "
                f"the pre-trigger window and then fires. Pass wait_for= (a "
        ...
        elif not callable(wait_for) and wait_for is not None and wait_for > 0:
            # Sequence 0 also gets the settle pause, during which the ring is
            # already filling. Later sequences get no such head start.
            head_start = _ARM_SETTLE_S if n_seq == 1 else 0.0
            if wait_for + head_start < fill_time:
                warnings.warn(
                    f"wait_for={wait_for} s is shorter than the {fill_time:.3f} s "
                    f"needed to fill the {pre_moi}-frame pre-trigger window, so "
                    f"the pre-trigger window will not be full.",
                    UserWarning,
                    stacklevel=2,
                )
        ...
        try:
            ...
            for seq_idx in range(n_seq):
                if seq_idx == 0:
                    ...
                    self._gvcp.write_reg(reg.REG_ACQUISITION_START, 1)
                    # The ring starts filling here, so the settle pause below
                    # already counts towards the pre-MOI window.
                ...
                self._wait_before_moi(wait_for, pre_moi, fill_time, verbose, armed_at)
    ...
    def _wait_before_moi(
        ...
        fill_time : float
            Seconds needed to fill the pre-trigger window.
        verbose : bool
            Print what is being waited for.
        armed_at : float or None, optional
            ``time.monotonic()`` reading taken when acquisition started, for
            the sequence that armed the camera.  The ring buffer fills from
            that moment, so the settle pause counts towards the pre-MOI
            window.  ``None`` for later sequences, which get no head start.
        """
        if callable(wait_for):
            if verbose:
                print("Waiting for wait_for()...", end=" ", flush=True)
            started = time.monotonic() if armed_at is None else armed_at
            wait_for()
            remaining = fill_time - (time.monotonic() - started)
            if remaining > 0:
                if verbose:
                    print(
        ...
        if wait_for is None:
            if pre_moi > 0 and fill_time > 0:
                elapsed = 0.0 if armed_at is None else time.monotonic() - armed_at
                remaining = max(0.0, fill_time - elapsed)
                if remaining > 0:
                    if verbose:
                        print(
                            f"Filling pre-MOI window ({pre_moi} frames, {remaining:.3f} s)...",
    ...
    def buffer_arm(self) -> None:
        ...
        fixes which part of the ring is kept.  Use this as the first step of
        the manual external-trigger workflow::
        ...
        The same flow works with a software MOI, which is the way to keep
        pre-trigger frames around an event that your own code detects::

            cam.buffer_configure(frames_per_seq=400, pre_moi=100)
            cam.buffer_arm()
            wait_for_my_event()     # the ring fills while you wait
            cam.buffer_fire_moi()
            cam.buffer_wait()

        :meth:`buffer_record` packs that flow into one call and takes the
        waiting step as its ``wait_for`` argument.
    ...
    def buffer_fire_moi(self) -> None:
        ...
        the pre-trigger window holds whatever the ring happened to contain::
    ...
    def buffer_moi_frame_id(self, sequence: int = 0) -> int:
        ...
        the ``start_frame`` argument of :meth:`buffer_download`.  Whether
        it also matches :attr:`pyTelops.FrameHeader.frame_id` is not yet
        verified on hardware, so do not rely on comparing the two.  Use
    ...
    def buffer_moi_index(self, sequence: int = 0) -> int:
        ...
        index of the moment of interest in the returned array, so it should
        come out equal to the configured ``pre_moi``.
        ...
        The mapping between the register id space and
        :attr:`pyTelops.FrameHeader.frame_id` is not yet verified on
        hardware.
